...
func portProfileSchema() map[string]*schema.Schema {
	return map[string]*schema.Schema{
		...
		"link_speed": {
			Description: "Link speed in Mbps. Can be set to `auto` for autonegotiation, or one of the following integers: `10`, `100`, `1000`, `2500`, `5000`, `10000`, `20000`, `25000`, `40000`, `50000` or `100000`",
			Type:        schema.TypeString,
			Optional:    true,
			Default:     "auto",
			ValidateFunc: validation.Any(
				validation.StringInSlice([]string{"auto"}, false),
				func(i interface{}, k string) (warnings []string, errors []error) {
					...
					f := validation.IntInSlice([]int{10, 100, 1000, 2500, 5000, 10000, 20000, 25000, 40000, 50000, 100000})
					return f(val, k)
				},
			),
		},
		...
		"lldpmed_enabled": {
			Description: "Enable Link Layer Discovery Protocol-Media Endpoint Discovery (LLDP-MED).",
			Type:        schema.TypeBool,
			Optional:    true,
			Default:     false,
		},
		...
		"native_network_id": {
			Description: "The ID of network to use as the main network on the port profile. Must be a valid network ID.",
			Type:        schema.TypeString,
			Optional:    true,
		},
		"name": {
		...
		"stp_enabled": {
			Description: "Enable spanning tree protocol (STP) for this port profile.",
			Type:        schema.TypeBool,
			Optional:    true,
			Default:     true,
		},
...
func resourcePortProfileGetResourceData(d *schema.ResourceData) (*unifi.PortProfile, error) {
	...
	taggedNetworkconfIds, err := setToStringSlice(d.Get("tagged_networks").(*schema.Set))
	if err != nil {
		return nil, err
	}

	port_profile := &unifi.PortProfile{
		Autoneg:                      true,
		...
		StpPortMode:                  d.Get("stp_enabled").(bool),
		TaggedNetworkIDs:             taggedNetworkconfIds,
		VoiceNetworkID:               d.Get("voice_networkconf_id").(string),
	}

	if speed, err := strconv.Atoi(d.Get("link_speed").(string)); err == nil {
		port_profile.Speed = speed
		port_profile.Autoneg = false
	}

	return port_profile, nil
}
...
func resourcePortProfileSetResourceData(resp *unifi.PortProfile, d *schema.ResourceData, site string) error {
	...
	d.Set("native_network_id", resp.NATiveNetworkID)
	...
	d.Set("stp_enabled", resp.StpPortMode)
	d.Set("tagged_networks", stringSliceToSet(resp.TaggedNetworkIDs))
	d.Set("voice_networkconf_id", resp.VoiceNetworkID)

	if resp.Autoneg {
		d.Set("link_speed", "auto")
	} else {
		d.Set("link_speed", strconv.Itoa(resp.Speed))
	}
...
func resourcePortProfileUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		...
		case *unifi.NotFoundError:
			return resourcePortProfileRead(d, meta)
		default:
			return err
		}
	} else {
